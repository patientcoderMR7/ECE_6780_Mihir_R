...
int led_button_toggle(void){
  ...
  //Configure PC6 and PC7 as outputs for LEDs
  GPIOC->MODER |= GPIO_MODER_MODER6_0 | GPIO_MODER_MODER7_0;
  GPIOC->OTYPER &= ~(GPIO_OTYPER_OT_6 | GPIO_OTYPER_OT_7);
  GPIOC->OSPEEDR &= ~(GPIO_OSPEEDR_OSPEEDR6 | GPIO_OSPEEDR_OSPEEDR7);
  GPIOC->PUPDR &= ~(GPIO_PUPDR_PUPDR6 | GPIO_PUPDR_PUPDR7);

    // Configure PA0 (B1 button) as input with pull-down resistor
        ...
        if (debouncer == 0xFFFFFFFF) {
            ...
            // This code triggers repeatedly when the button is steady low!
							...
							GPIOC->ODR ^= (GPIO_ODR_6 | GPIO_ODR_7);
								...
								HAL_Delay(20);// Add a delay to avoid multiple toggles on a single press
				}
			}
